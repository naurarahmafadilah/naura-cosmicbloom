...
import { 
  ...
  FaBoxOpen, 
  FaTag,
  FaIdCard // Import ikon kartu/membership tambahan jika dibutuhkan
} from "react-icons/fa";
import { 
  FiUsers, 
  FiTrendingUp, 
  ...
  const adminSections = [
    ...
    },
    {
      ...
      items: [
        ...
        // MENAMBAHKAN MENU MEMBERSHIP DI SINI
              ...
              {section.items.map((item, i) => (
                <li key={i}>
                  <NavLink to={item.path} end={item.end} className={menuClass}>
                    {/* Menggunakan render props ({ isActive }) dari NavLink untuk mengatur warna ikon internal */}
                    {({ isActive }) => (
                      <>
                        <div className="flex items-center gap-3">
                          <span className={`text-sm shrink-0 transition-colors ${
                            isActive 
                              ? 'text-white' // Saat aktif, paksa ikon menjadi putih bersih agar kelihatan terang
                              : item.isSale 

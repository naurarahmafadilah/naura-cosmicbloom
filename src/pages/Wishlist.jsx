...
// Import menggunakan nama file kecil untuk konsistensi
  ...
  return (
    <div className="space-y-12 animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[2px] text-secondary-dark/40 bg-bg-soft px-4 py-2 rounded-full w-fit">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {wishlistData.map((item) => (
          <div key={item.id} className="group relative flex flex-col h-full">
            <div className="relative overflow-hidden rounded-[40px] aspect-[4/5] bg-bg-main shadow-sm group-hover:shadow-veloura transition-all duration-500 border border-bg-soft">
              ...
              <div className="absolute bottom-5 left-1/2 -translate-x-1/2 w-[80%] bg-white/80 backdrop-blur-sm py-2 rounded-2xl text-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
            ...
            <div className="mt-6 flex-1 text-center space-y-2">
              <Link to={`/wishlist/${item.slug}`}>
                <h3 className="text-lg font-playfair text-primary-dark group-hover:text-secondary-light transition-colors duration-300 line-clamp-1 px-2">
                  {item.name}
                </h3>
              </Link>
        ...
        <Link 
          to="/shop"
          className="border-2 border-dashed border-bg-soft rounded-[40px] flex flex-col items-center justify-center p-10 min-h-[400px] hover:border-secondary-light/30 transition-all cursor-pointer group"
        >
